...
BASEURL=https://files.freeswitch.org/releases
...
get_sound_version() {
    local SOUND_TYPE=$1
    grep "$SOUND_TYPE" $SCRIPT_DIR/sounds_version.txt | sed -E "s/$SOUND_TYPE\s+//"
}
...
download_sound_rates() {
    local i
    local f
    local SOUND_VERSION=$1
    local SOUND_LOCALE=$2
    local MUSIC_VERSION="1.0.7"
    for i in $SOUND_RATES
    do
        f="sounds/freeswitch-sounds-$SOUND_LOCALE-$i-$SOUND_VERSION.tar.gz"
        echo "Downloading $f"
        wget_helper $f
        g="music/freeswitch-sounds-music-$i-$MUSIC_VERSION.tar.gz"
        echo "Downloading $g"
        wget_helper $g
    done
}
...
extract_sound_files() {
    ...
    local MUSIC_FILES=freeswitch-music-*.tar.gz
    for f in $MUSIC_FILES
    do
        if [ -f $f ]; then
            echo "Extracting file $f"
            tar xzf $f -C /usr/share/freeswitch/sounds/music/
        fi
    done
}

delete_archives() {
    local FILES_COUNT=$(ls -1 freeswitch-{sounds,music}-*.tar.gz 2> /dev/null | wc -l)
